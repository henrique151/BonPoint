...
const ChatScreen: React.FC<{ idConfeitaria: string }> = ({ idConfeitaria }) => {
      ...
      try {
        ...
        if (bloqueiosDoc.exists()) {
          ...
          if (bloqueados.includes(idUsuarioDestinatario)) {
            ...
            // Toast.show("Você bloqueou este usuário.", {
            //   type: "success",
            // });
          } else {
  ...
  const denunciarUsuario = async () => {
    ...
    try {
      const response = await fetch("http://192.168.1.5:3000/denunciar", {
        method: "POST",
        headers: {
  ...
  const enviarMensagem = async () => {
    if (sendingMessage) return; // Evita múltiplos envios enquanto uma mensagem está sendo enviada
    setSendingMessage(true); // Marca que uma mensagem está sendo enviada

    if (!usuarioLogado || !idUsuarioDestinatario) {
      console.error("Usuário ou destinatário não estão definidos.");
      setSendingMessage(false); // Libera o botão caso haja erro
      return;
    }

    if (usuarioBloqueado) {
      setSendingMessage(false); // Libera o botão se o usuário estiver bloqueado
      return;
    }
    ...
    if (novaMensagemObj.texto === "" && imagensSelecionadas.length === 0) {
      console.error("Mensagem não pode ser vazia.");
      setSendingMessage(false); // Libera o botão caso a mensagem seja vazia
      return;
    }
    ...
    try {
      ...
      // Envio de notificação se ainda não atingiu o limite
      if (notificationCount < 2) {
        ...
        if (destinatarioDoc.exists()) {
          ...
          if (tokenDestinatario) {
            await fetch("http://192.168.1.5:3000/send-notification", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
            ...
            setNotificationCount((prevCount) => prevCount + 1); // Incrementa o contador
          } else {
            console.error("Token do destinatário não encontrado.");
          }
        } else {
      ...
      setSendingMessage(false); // Libera o botão após o envio ser concluído
    }
  };
